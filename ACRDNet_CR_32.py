...
class ACRNet(nn.Module):
    def __init__(self,
        ...
        self.fully_connect_layer=nn.Linear(total_size // reduction,64)
        self.sigmoid_sparse=nn.Sigmoid()
        self.fully_connect_layer_2=nn.Linear(64,128)
        self.sigmoid_sparse=nn.Sigmoid()
        self.fully_connect_layer_3=nn.Linear(128,256)
        self.sigmoid_sparse=nn.Sigmoid()
        self.fully_connect_layer_4=nn.Linear(256,512)
        self.sigmoid_sparse=nn.Sigmoid()
        self.fully_connect_layer_5=nn.Linear(512,1024)
        self.sigmoid_sparse=nn.Sigmoid()
        self.fully_connect_layer_6=nn.Linear(1024,512)
        self.sigmoid_sparse=nn.Sigmoid()
        self.fully_connect_layer_7=nn.Linear(512,256)
        self.sigmoid_sparse=nn.Sigmoid()
        self.fully_connect_layer_8=nn.Linear(256,128)
        self.sigmoid_sparse=nn.Sigmoid()
        self.fully_connect_layer_9=nn.Linear(128,64)
        #self.sigmoid_sparse=nn.Sigmoid()
        #self.fully_connect_layer_10=nn.Linear(64,32)
        # self.sigmoid_sparse=nn.Sigmoid()
        # self.fully_connect_layer_11=nn.Linear(64,32)
    ...
    def forward(self, x):
        ...
        out.to_sparse_csr()
        #fully_connected_layer
        out=self.fully_connect_layer_7(out)
        #activation
        out=self.sigmoid_sparse(out)
        out.to_sparse_csr()
        #fully_connected_layer
        out=self.fully_connect_layer_8(out)
        #activation
        out=self.sigmoid_sparse(out)
        out.to_sparse_csr()
        #fully_connected_layer
        out=self.fully_connect_layer_9(out)
        #activation
        #out=self.sigmoid_sparse(out)
        #out.to_sparse_csr()
        #fully_connected_layer
        #out=self.fully_connect_layer_10(out)
        #activation
        # out=self.sigmoid_sparse(out)
        # out.to_sparse_csr()
        # #fully_connected_layer
        # out=self.fully_connect_layer_11(out)
